                ...
                <div class="card">
                    ...
                    <div class="card-body py-4" dir="{{Config::get('app.locale') == 'en' ? 'ltr' : 'rtl'}}">
                        ...
                        <div class="row px-0">
                            ...
                            @if(isset($data->hotelImages) && $data->hotelImages->count() > 0)
                            @foreach($data->hotelImages as $image)

                                <div class="col-lg-3 col-md-3 col-sm-4 col-6 m-auto text-center position-relative">
                                    <div class="delete_image text-danger" data-target="{{$image->id}}" style="cursor:pointer;position: absolute; width: 26px; height: 25px; border-radius: 5px; background: #fff;top: 12px; right: 20px">
                                    ...
                                    <img src="{{asset('uploads/hotels/'. $image->image)}}" alt="image" class="w-100">
                                    </a>
                                </div>
                            @endforeach
                                @else
                ...
                if (id !== "") {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "/admin/hotels/delete-images/" + id,
                                data: {
                                    _token: "{{csrf_token()}}",
                                    _method: "POST",
                                    id: id,
